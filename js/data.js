...
function normalize(value){
  ...
  next.syncSuggestions.forEach(item=>{item.id=String(item.id||uid('sg'));item.source=['gmail','calendar','sms'].includes(item.source)?item.source:'gmail';item.category=['bills','travel','school','health','deliveries','home','government'].includes(item.category)?item.category:'home';item.title=String(item.title||'Imported update').slice(0,160);item.summary=String(item.summary||'').slice(0,300);item.sender=String(item.sender||'').slice(0,100);item.receivedAt=String(item.receivedAt||'');item.actionDate=String(item.actionDate||'').slice(0,10);item.urgency=['high','medium','normal'].includes(item.urgency)?item.urgency:'normal';item.decision=String(item.decision||'Review').slice(0,80);item.processedAt=String(item.processedAt||item.receivedAt||'');item.appliedAt=String(item.appliedAt||'');item.trusted=Boolean(item.trusted);item.amount=Math.max(0,+item.amount||0);item.status=['pending','applied','dismissed'].includes(item.status)?item.status:'pending';item.sourceRef=String(item.sourceRef||item.id).slice(0,180);item.personId=String(item.personId||'')});
