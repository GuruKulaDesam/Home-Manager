...
function normalize(value){
  ...
  next.settings={
    ...
    householdName:typeof inputSettings.householdName==='string'&&inputSettings.householdName.trim()?inputSettings.householdName.trim():'Lotus Naga Home',
    language:typeof inputSettings.language==='string'?inputSettings.language:'English',
    primaryAddress:typeof inputSettings.primaryAddress==='string'&&inputSettings.primaryAddress.trim()?inputSettings.primaryAddress.trim():'32 SSS Jaya Enclave, Kovaipudur, Coimbatore, 641042',
  ...
  next.people.forEach(item=>{
    item.wellbeing=Math.min(100,Math.max(0,+item.wellbeing||0));
    if(item.id==='p3'&&/^student$/i.test(String(item.householdRole||'')))item.householdRole='Daughter';
    if(item.id==='p4'&&/^student$/i.test(String(item.householdRole||'')))item.householdRole='Son';
  });
  ...
  next.syncSuggestions.forEach(item=>{item.id=String(item.id||uid('sg'));item.source=['gmail','calendar','sms'].includes(item.source)?item.source:'gmail';item.category=['bills','travel','school','health','deliveries','home','government'].includes(item.category)?item.category:'home';item.title=String(item.title||'Imported update').slice(0,160);item.summary=String(item.summary||'').slice(0,600);item.sender=String(item.sender||'').slice(0,100);item.receivedAt=String(item.receivedAt||'');item.actionDate=String(item.actionDate||'').slice(0,10);item.urgency=['high','medium','normal'].includes(item.urgency)?item.urgency:'normal';item.decision=String(item.decision||'Review').slice(0,80);item.processedAt=String(item.processedAt||item.receivedAt||'');item.appliedAt=String(item.appliedAt||'');item.trusted=Boolean(item.trusted);item.amount=Math.max(0,+item.amount||0);item.transactionType=['expense','credit','due','notice'].includes(item.transactionType)?item.transactionType:'notice';item.status=['pending','applied','dismissed'].includes(item.status)?item.status:'pending';item.sourceRef=String(item.sourceRef||item.id).slice(0,180);item.personId=String(item.personId||'')});
  next.quickNotes.forEach(item=>{item.id=String(item.id||uid('qn'));item.text=String(item.text||'').slice(0,500);item.ownerId=String(item.ownerId||'');item.createdAt=String(item.createdAt||new Date().toISOString());item.status=item.status==='archived'?'archived':'active'});
