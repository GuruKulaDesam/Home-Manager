(function () {
  ...
  function appSettings(intro, settings) {
    ...
    return `${intro}
      ...
      <section class="panel integration-queue"><div class="section-head"><div><span class="section-kicker">REVIEW BEFORE APPLYING</span><h2>Google and SMS updates</h2><p>Nothing changes household records until a family member approves it.</p></div><div class="toolbar-actions"><span class="sync-state ${pending.length ? 'pending' : 'connected'}">${pending.length} pending</span><button type="button" data-route="global/intelligence">${icon('chart-no-axes-combined')}<span>Inbox report</span></button></div></div><div class="suggestion-list">${queue}</div></section>
