(function () {
  ...
  function chapterWorkspace(lessonId, section = 'learn') {
    ...
    return `<div class="chapter-workspace-shell"><aside class="chapter-workspace-sidebar"><header class="chapter-workspace-head"><span class="context-badge study">${jeeMode ? 'JEE Main' : 'CBSE'}</span><h1 id="chapterWorkspaceTitle">${e(lesson.title)}</h1><p>${e(lesson.subject)} · ${e(lesson.term)}</p></header><nav class="chapter-workspace-tabs" aria-label="Chapter learning journey">${tabs.map(([value,label,iconName]) => `<button type="button" data-chapter-workspace-tab="${value}" data-lesson="${e(lesson.id)}" class="${section === value ? 'active' : ''}" ${section === value ? 'aria-current="page"' : ''}>${icon(iconName)}<span>${label}</span></button>`).join('')}</nav><div class="chapter-workspace-status"><span>Mastery</span><b>${mastery}%</b></div><button type="button" class="chapter-workspace-close" data-close-chapter-workspace>${icon('arrow-left')}<span>All chapters</span></button></aside><main class="chapter-workspace-content chapter-section-${e(section)}">${panels[section] || learn}</main></div>`;
  }
