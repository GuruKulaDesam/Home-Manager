(function () {
  ...
  function family() {
    const s = D.state;
    return `<div class="toolbar"><span>${s.people.length} family members</span><span class="grow"></span><button data-route="settings/people">${icon('users-round')}<span>Manage profiles</span></button></div><div class="cards">${s.people.map(person => `<article class="card"><span class="avatar">${e((person.name || '?')[0])}</span><h3>${e(person.name)}</h3><p>${e(person.householdRole)}</p><div class="row"><div class="grow"><small>Current wellbeing</small><b>${clamp(person.wellbeing)}%</b></div></div></article>`).join('')}</div>${sectionFinance('family', ['income', 'goal', 'tax', 'insurance'])}`;
  }
  ...
  const lifeFinanceDomains = { property: 'housing', bills: 'housing', subscriptions: 'housing', digital: 'housing', help: 'housing', sustainability: 'housing', vehicles: 'vehicle', health: 'health', emergency: 'health', pets: 'health', travel: 'family', festivals: 'family', documents: 'family', tax: 'family', insurance: 'family', legacy: 'family', education: 'learning' };
  ...
  const monthlyValue = item => (+item.amount || 0) * ({ Monthly: 1, Quarterly: 1 / 3, 'Half-yearly': 1 / 6, Yearly: 1 / 12, 'One time': 0, 'As needed': 0 }[item.frequency] ?? 1);
