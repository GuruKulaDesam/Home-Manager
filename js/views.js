(function () {
  ...
  const titles = {
    ...
    'study/curriculum': ['Learning Path', 'One connected journey through every curriculum chapter'],
  ...
  function curriculumJourney() {
    ...
    const rows = lessons.map((lesson, index) => {
      ...
      const stage = mastery >= 80 ? 'Mastered' : mastery >= 50 ? 'Learning' : mastery ? 'Started' : 'Ready';
      return `<article class="curriculum-journey-row chapter-tone-${index % 6 + 1}" data-filter-row><span class="chapter-sequence">${String(index + 1).padStart(2, '0')}</span><div class="chapter-journey-copy"><span class="section-kicker">${e(lesson.term)} · ${e(lesson.competency)}</span><h2>${e(lesson.title)}</h2><div class="chapter-progress-line"><span><i style="width:${clamp(mastery)}%"></i></span><b>${mastery}% · ${stage}</b></div></div><div class="chapter-journey-actions" aria-label="Open ${e(lesson.title)} workspace"><button type="button" class="chapter-primary-action" data-chapter-workspace="${e(lesson.id)}" data-chapter-section="learn">${icon('sparkles')}<span>Study chapter</span></button><button type="button" data-chapter-workspace="${e(lesson.id)}" data-chapter-section="book" aria-label="Open book for ${e(lesson.title)}" title="Book">${icon('book-open')}</button><button type="button" data-chapter-workspace="${e(lesson.id)}" data-chapter-section="practice" aria-label="Open guided questions for ${e(lesson.title)}" title="Guided questions">${icon('list-checks')}</button><button type="button" data-chapter-workspace="${e(lesson.id)}" data-chapter-section="assignments" aria-label="Open assignments for ${e(lesson.title)}" title="Assignments">${icon('clipboard-check')}</button><button type="button" data-chapter-workspace="${e(lesson.id)}" data-chapter-section="progress" aria-label="Open progress for ${e(lesson.title)}" title="Progress">${icon('chart-no-axes-combined')}</button></div></article>`;
    }).join('');
  ...
  function chapterWorkspace(lessonId, section = 'learn') {
    ...
    const bookPanel = bookEntry ? `<section class="chapter-book-panel"><header><div><span class="section-kicker">${e(bookEntry.book.publisher)} · OFFLINE</span><h2>${e(bookEntry.part.label)}</h2><p>${e(bookEntry.book.title)}</p></div><a href="${e(bookEntry.book.sourceUrl)}" target="_blank" rel="noopener noreferrer">${icon('external-link')} Official source</a></header><iframe title="${e(bookEntry.part.label)} textbook section" src="${e(`${bookEntry.part.url}#view=FitH`)}"></iframe></section>` : `<section class="chapter-empty-state">${icon('book-x')}<h2>No exact book section is mapped yet</h2><p>The teacher notes and guided questions remain available.</p></section>`;
    ...
    const panels = { learn, book: bookPanel, practice, assignments: assignmentPanel, exam: examPanel, progress: progressPanel };
    const tabs = [['learn','Learn','sparkles'],['book','Book','book-open'],['practice','Guided Questions','list-checks'],['assignments','Assignments','clipboard-check'],['exam','Exam Tips','badge-check'],['progress','Progress','chart-no-axes-combined']];
    return `<header class="chapter-workspace-head"><div><span class="context-badge study">${jeeMode ? 'JEE Main' : 'CBSE'} · ${e(lesson.subject)}</span><h1 id="chapterWorkspaceTitle">${e(lesson.title)}</h1><p>${e(lesson.term)} · ${mastery}% mastery</p></div><button type="button" data-close-dialog="chapterWorkspaceDialog" aria-label="Close chapter workspace">${icon('x')}</button></header><nav class="chapter-workspace-tabs" aria-label="Chapter learning journey">${tabs.map(([value,label,iconName]) => `<button type="button" data-chapter-workspace-tab="${value}" data-lesson="${e(lesson.id)}" class="${section === value ? 'active' : ''}">${icon(iconName)}<span>${label}</span></button>`).join('')}</nav><main class="chapter-workspace-content">${panels[section] || learn}</main>`;
  }
