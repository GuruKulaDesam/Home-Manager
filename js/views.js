(function () {
  ...
  const financeDomains = {
    ...
    learning: { label: 'Learning', route: 'study/overview', icon: 'graduation-cap' },
  ...
  function appSettings(intro, settings) {
    ...
    const googleSlots = [0, 1].map(index => accounts[index] || { slotId: `google-${index + 1}`, personId: D.state.people[index]?.id || D.state.people[0]?.id || '', email: '', consent: false, status: 'pending' });
    const categories = [
    ...
    return `${intro}
      ...
      <form id="googleSyncSettings" class="panel sync-settings"><div class="section-head"><div><span class="section-kicker">GOOGLE ACCOUNTS</span><h2>Gmail, Calendar and Drive</h2><p>Connect both Google accounts separately. Imported updates always enter the review queue.</p></div><span class="sync-state ${connectorReady ? connected ? 'connected' : 'pending' : 'required'}">${connectorReady ? connected ? `${connected} connected` : 'Awaiting accounts' : 'Secure connector required'}</span></div>
        ...
        <div class="sync-controls"><label>Look back<select name="lookbackDays"><option value="7" ${+sync.lookbackDays === 7 ? 'selected' : ''}>7 days</option><option value="30" ${+sync.lookbackDays === 30 ? 'selected' : ''}>30 days</option><option value="90" ${+sync.lookbackDays === 90 ? 'selected' : ''}>90 days</option></select></label><label>Apply policy<select name="reviewPolicy"><option value="review" selected>Review every suggestion</option></select></label><button type="button" data-google-status ${connectorReady ? '' : 'disabled'}>${icon('cloud-cog')}<span>Check accounts</span></button><button type="button" data-google-sync ${connectorReady && connected ? '' : 'disabled'}>${icon('refresh-cw')}<span>Sync now</span></button><button type="submit" class="primary">${icon('save')}<span>Save</span></button></div>
      </form>
