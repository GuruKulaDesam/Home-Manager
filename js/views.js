(function () {
  ...
  const groups = {
    ...
    learning: { label: 'Education', icon: 'graduation-cap', note: 'Study and development', route: 'study/curriculum', items: [
      ['Curriculum', 'route', 'study/curriculum'], ['Genius Mind', 'brain', 'study/genius'], ['Planner', 'calendar-clock', 'study/planner'], ['Overview', 'activity', 'study/overview'], ['Progress', 'chart-no-axes-combined', 'study/reports']
    ]},
  ...
  function gmailEssence(categoryKeys, title, note, showEmpty = false) {
    const signals = (D.state.syncSuggestions || []).filter(item => item.source === 'gmail' && categoryKeys.includes(item.category)).sort((a, b) => String(b.receivedAt).localeCompare(String(a.receivedAt)));
    ...
    return `<section class="panel module-inbox-brief"><div class="section-head"><div><span class="section-kicker">FAMILY EMAIL ESSENCE</span><h2>${e(title)}</h2><p>${signals.length} relevant messages · ${D.money(detected)} detected · ${e(note)}</p></div><button data-route="global/intelligence">All family email</button></div>${content}</section>`;
  }
  ...
  function homeOverview() {
    ...
    return `<section class="metrics">${metric('Open responsibilities', openTasks.length, openTasks.filter(item => item.dueAt < today()).length ? 'Includes overdue work' : 'Household handoff', 'list-todo')}${metric('Low stock', lowStock.length, nextMeal ? `Next meal ${nextMeal.name}` : 'No meal planned', 'shopping-basket')}${metric('Open repairs', issues.length, 'Property and appliances', 'wrench')}${metric('Monthly home spend', D.money(spend), month, 'wallet-cards')}</section><div class="grid-2"><section class="panel"><div class="section-head"><div><h2>Run the home next</h2><p>Responsibilities sorted by due date</p></div><button data-route="home/tasks">All tasks</button></div>${openTasks.length ? openTasks.slice(0, 7).map(x => row(x.title, `${x.assignee || 'Unassigned'} - ${D.date(x.dueAt)}`, status(x.status))).join('') : empty('No home tasks need attention.', 'task', 'Add task')}</section><section class="panel"><div class="section-head"><div><h2>Services and upkeep</h2><p>Renewals, repairs and recurring support</p></div><button data-route="home/property">Property</button></div>${issues.slice(0, 3).map(item => row(item.title, `${item.category} - ${item.location}`, `<button data-route="home/property">Open</button>`)).join('')}${services.slice(0, 4).map(item => row(item.title, `${HM.life.domains[item.domain].title} - ${D.date(item.dueDate)}`, `<button data-route="home/life/${item.domain}">Open</button>`)).join('') || '<p class="empty">No service or upkeep item needs attention.</p>'}</section></div><section class="family-action-grid household-action-grid" aria-label="Household workflows">${[['layout-dashboard','Home overview','Readiness and handoffs','home/overview'],['list-checks','Tasks & routines','Assigned and recurring work','home/tasks'],['shopping-basket','Food & supplies','Meals, pantry and shopping','home/inventory'],['wrench','Property & assets','Repairs, warranties and value','home/property'],['car-front','Vehicles','Service, fuel and renewals','home/life/vehicles'],['hand-helping','Domestic help','Attendance, pay and contacts','home/life/help'],['leaf','Sustainability','Water, energy, waste and garden','home/life/sustainability']].map(item => `<button data-route="${item[3]}"><span>${icon(item[0])}</span><span><b>${item[1]}</b><small>${item[2]}</small></span>${icon('chevron-right')}</button>`).join('')}</section>`;
  }
  ...
  function family() {
    ...
    const openTasks = s.tasks.filter(item => item.context === 'home' && D.status(item.status) !== 'done');
    ...
    const averageWellbeing = s.people.length ? Math.round(s.people.reduce((total, person) => total + clamp(person.wellbeing), 0) / s.people.length) : 0;
    return `<section class="metrics compact-metrics">${metric('Family members', s.people.length, 'Profiles and roles', 'users-round')}${metric('Family wellbeing', `${averageWellbeing}%`, 'Conversation prompt, not diagnosis', 'heart-handshake')}${metric('Next 30 days', upcoming.filter(item => isoDay(item.startAt) <= new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10)).length, 'Shared events', 'calendar-days')}${metric('Continuity records', continuity.length, 'Documents, cover and legacy', 'shield-check')}</section>
      <div class="family-suite-grid"><section class="panel"><div class="section-head"><div><h2>Family pulse</h2><p>Check load and wellbeing without ranking people</p></div><button data-route="settings/people">${icon('user-cog')}<span>Profiles</span></button></div>${s.people.map((person, index) => { const taskCount = openTasks.filter(task => task.assignee === person.name).length; return `<div class="family-pulse-row"><span class="member-avatar person-${index % 6}">${e((person.name || '?')[0])}</span><div class="grow"><b>${e(person.name)}</b><small>${e(person.householdRole)} - ${taskCount} open task${taskCount === 1 ? '' : 's'}</small><div class="progress"><span style="width:${clamp(person.wellbeing)}%"></span></div></div><strong>${clamp(person.wellbeing)}%</strong></div>`; }).join('')}</section>
  ...
  function careOverview() {
    ...
    return `<section class="care-readiness"><div><span class="section-kicker">FAMILY CARE PLAN</span><h2>One handoff for health and safety</h2><p>Keep treatment tasks, refills, consultations, elder support and emergency instructions visible to the family member responsible.</p></div><div class="care-readiness-score"><strong>${Math.min(100, 40 + emergency.length * 20 + Math.min(20, medicines.length * 10) + Math.min(20, appointments.length * 10))}%</strong><small>setup readiness</small></div></section>
  ...
  function learnerBar(context) {
    ...
    const bar = `<section class="learner-bar"><div class="learner-switch" role="group" aria-label="Choose student">${context.profiles.map((profile, index) => `<button data-learner="${e(profile.personId)}" class="student-tone-${index + 1} ${profile.personId === context.activeId ? 'active' : ''}" aria-pressed="${profile.personId === context.activeId}" title="${e(profile.name)} · Class ${e(profile.grade)}"><span>${e(profile.name[0])}</span><b>${e(profile.name.split(' ')[0])}</b></button>`).join('')}</div></section>`;
    ...
    const cbseSections = [['Curriculum','route','study/curriculum'],['Genius Mind','brain','study/genius'],['Planner','calendar-clock','study/planner'],['Overview','activity','study/overview'],['Progress','chart-no-axes-combined','study/reports']];
    ...
    return `<div class="learning-command-bar subject-first"><div class="education-command-row">${bar}<div class="education-master-controls">${trackTabs}${subjectTabs}</div>${sectionTabs}</div></div>${extension}`;
  }
  ...
  const chapterStages = [
    ...
    ['understand', 'Understand', 'lightbulb'],
    ['book', 'Read Book', 'book-open'],
    ['notes', 'My Notes', 'notebook-pen'],
    ['practice', 'Practice', 'list-checks'],
  ...
  function chapterWorkspace(lessonId, section = 'summary') {
    ...
    const teachingSteps = (values, start = 1, formulaAware = false) => `<div class="chapter-slow-steps">${(values || []).map((value, index) => { const isFormula = formulaAware && formulaPattern.test(String(value)); return `<article class="${isFormula ? 'chapter-formula-card' : ''}"><span>${isFormula ? icon('sigma') : start + index}</span><div><p>${e(value)}</p>${isFormula ? '<small>Read the relationship from left to right, then identify every symbol before substituting numbers.</small>' : ''}</div></article>`; }).join('')}</div>`;
    const wordCards = (foundation.newWords || []).map((word, index) => { const item = typeof word === 'string' ? { term: word, plain: '' } : word; return `<article><span>${index + 1}</span><div><h3>${e(item.term)}</h3><p>${e(item.plain || 'This word names one of the chapter’s central ideas.')}</p></div></article>`; }).join('');
    ...
    const relationshipCaption = foundation.visual?.caption || `A relationship map for ${lesson.title}. Follow the numbered boxes in order.`;
    ...
    const progressPanel = `<section class="chapter-progress-panel"><div class="chapter-mastery-ring" style="--mastery:${clamp(mastery)}"><strong>${mastery}%</strong><span>${e(statusValue.replace('-', ' '))}</span></div><div><span class="section-kicker">RECORD REAL MASTERY</span><h2>How securely can you use this chapter?</h2><p>Choose the strongest statement that is honestly true. Mastery completes at 80% or higher.</p><div class="mastery-choices">${[[20,'I recognise it'],[40,'I can explain it'],[60,'I can solve standard questions'],[80,'I can handle unfamiliar questions'],[100,'I can teach and verify it']].map(([value, label]) => `<button type="button" data-chapter-mastery="${value}" data-lesson="${e(lesson.id)}" data-jee="${jeeMode}" class="${mastery === value ? 'active' : ''}"><b>${value}%</b><span>${label}</span></button>`).join('')}</div></div>${sevenStatus}</section>`;
    const memoryBySubject = {
      Physics: ['Draw the situation before recalling an equation.', 'Say what each symbol measures and include its unit.', 'Link cause → law → observable result.'],
      Chemistry: ['Group reactions by the change they produce.', 'Rebuild each equation from reactant → condition → product.', 'Use one contrast table for similar terms and exceptions.'],
      Mathematics: ['Recreate the definition with one example and one non-example.', 'Connect each formula to its graph or geometric meaning.', 'Solve one tiny case before using the general rule.'],
      'Computer Science': ['Trace one small input through every step.', 'Remember the purpose before the syntax.', 'Compare the normal path with one error case.'],
      'English Core': ['Attach each idea to one exact textual detail.', 'Retell the change as beginning → turning point → result.', 'Use character, conflict and consequence as recall anchors.']
    };
    ...
    const memoryMethods = memoryBySubject[lesson.subject] || ['Turn the main relationship into a three-step story.', 'Cover the page and rebuild the visual from memory.', 'Explain one example aloud without using the textbook words.'];
    const errorChecks = {
      Physics: ['Check the direction before the magnitude.', 'Test the unit and the limiting case.', 'Make sure the chosen law matches the symmetry or conditions.'],
      Chemistry: ['Balance atoms and charge before calculating.', 'Write the condition above the reaction arrow.', 'Check whether the named exception changes the expected product or trend.'],
      Mathematics: ['State the allowed domain or condition.', 'Substitute the result back into the original statement.', 'Use a graph, sign or size check to reject an impossible answer.'],
      'Computer Science': ['Trace the smallest valid input.', 'Check data type, boundary and failure path.', 'Separate a syntax error from a logic error.'],
      'English Core': ['Tie every claim to a precise detail.', 'Keep the speaker, narrator and author distinct.', 'Explain how the evidence supports the conclusion.']
    }[lesson.subject] || ['Check the meaning, the evidence and the final conclusion.', "Compare the answer with the chapter's central relationship."];
    ...
    if ((summary.rapidRecall || []).length >= 2) contextualSupports.push(supportCard('remember', 'Do Not Forget This', 'pin', ['opening','results','recall'], supportList(summary.rapidRecall)));
    ...
    const summaryWithSupport = `<div class="chapter-summary-layout">${summaryPanel}${supportRail}</div>`;
    const learningWithSupport = `<div class="chapter-summary-layout chapter-understand-layout">${learn}${supportRail}</div>`;
    const panels = { summary: summaryWithSupport, understand: learningWithSupport, book: bookPanel, notes: notePanel, practice, assignments: assignmentPanel, progress: progressPanel };
    const canToggle = !['notes', 'progress'].includes(section);
    const stageAction = canToggle ? `<button type="button" class="chapter-stage-toggle compact ${tracking.status[section] ? 'complete' : ''}" data-chapter-stage-toggle="${e(section)}" data-lesson="${e(lesson.id)}">${icon(tracking.status[section] ? 'circle-check-big' : 'circle')}<span>${tracking.status[section] ? 'Completed' : 'Mark complete'}</span></button>` : `<span class="chapter-tab-context">${icon(tracking.status[section] ? 'circle-check-big' : 'info')}<b>${section === 'notes' ? 'Save a note to complete' : '80% mastery completes this stage'}</b></span>`;
    const workspaceTabs = `<header class="chapter-content-tabs"><nav class="chapter-workspace-tabs" aria-label="Chapter learning sections">${chapterSections.map(([value,label,iconName]) => `<button type="button" data-chapter-workspace-tab="${value}" data-lesson="${e(lesson.id)}" class="${section === value ? 'active' : ''} ${value !== 'summary' && tracking.status[value] ? 'complete' : ''}" ${section === value ? 'aria-current="page"' : ''}>${icon(iconName)}<span>${label}</span>${value !== 'summary' && tracking.status[value] ? icon('circle-check-big') : ''}</button>`).join('')}</nav>${stageAction}</header>`;
    return `<div class="chapter-workspace-shell">${workspaceTabs}<main class="chapter-workspace-content chapter-section-${e(section)}">${panels[section] || summaryPanel}</main></div>`;
  }
