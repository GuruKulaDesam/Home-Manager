(function () {
  ...
  function chapterWorkspace(lessonId, section = 'summary') {
    ...
    const resourceConfig = lesson.subject === 'Computer Science'
      ? { label: 'Syntax & Patterns', title: 'Syntax and Patterns You Must Be Able to Use', item: 'Pattern', icon: 'code-2', formula: false }
      : languageSubject
    ...
    const resourceCards = resourceItems.map((value, index) => {
      ...
      const caution = (summary.examTraps || guide.traps || [])[index % Math.max((summary.examTraps || guide.traps || []).length, 1)] || 'Do not apply the relationship without checking its conditions.';
    ...
    const resourcePanel = `<section class="chapter-resource-panel ${resourceConfig.formula ? 'formulae' : 'conceptual'}"><header><span>${icon(resourceConfig.icon)}</span><div><small>${e(resourceConfig.label.toUpperCase())}</small><h1>${e(resourceConfig.title)}</h1><p>${resourceConfig.formula ? 'Read every formula as a relationship between quantities. Learn the meaning and conditions before substituting values.' : 'Use each tool with its purpose, evidence and limits clear.'}</p></div></header><div class="chapter-resource-cards">${resourceCards || '<p class="empty">This chapter uses its central relationships directly in the guided examples.</p>'}</div></section>`;
    ...
    const chapterReferenceRail = `<section class="chapter-reference-rail" aria-label="Chapter foundations and vocabulary"><article class="chapter-reference-card foundation"><header><span>1</span><h2>Bring These Ideas with You</h2></header>${supportList(foundation.remember)}</article><article class="chapter-reference-card vocabulary"><header><span>2</span><h2>New Words, in Plain Words</h2></header><dl>${(foundation.newWords || []).map(word => { const item = typeof word === 'string' ? { term: word, plain: '' } : word; return `<div><dt>${e(titleCase(item.term))}</dt><dd>${e(item.plain || 'A central idea used in this chapter.')}</dd></div>`; }).join('')}</dl></article></section>`;
    ...
    if ((summary.rapidRecall || []).length >= 2) contextualSupports.push(supportCard('remember', 'Do Not Forget This', 'pin', ['results','recall'], supportList(summary.rapidRecall)));
