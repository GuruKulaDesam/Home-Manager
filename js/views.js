(function () {
  ...
  function chapterWorkspace(lessonId, section = 'learn') {
    ...
    return `<div class="chapter-workspace-top"><header class="chapter-workspace-head"><span class="context-badge study">${jeeMode ? 'JEE Main' : 'CBSE'} · ${e(lesson.subject)}</span><h1 id="chapterWorkspaceTitle">${e(lesson.title)}</h1><p>${e(lesson.term)} · ${mastery}%</p></header><nav class="chapter-workspace-tabs" aria-label="Chapter learning journey">${tabs.map(([value,label,iconName]) => `<button type="button" data-chapter-workspace-tab="${value}" data-lesson="${e(lesson.id)}" class="${section === value ? 'active' : ''}">${icon(iconName)}<span>${label}</span></button>`).join('')}</nav><button type="button" class="chapter-workspace-close" data-close-dialog="chapterWorkspaceDialog" aria-label="Close chapter workspace">${icon('x')}</button></div><main class="chapter-workspace-content chapter-section-${e(section)}">${panels[section] || learn}</main>`;
  }
