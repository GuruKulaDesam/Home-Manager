(function () {
  ...
  function appSettings(intro, settings) {
    ...
    return `${intro}
      ...
      <form id="googleSyncSettings" class="panel sync-settings"><div class="section-head"><div><span class="section-kicker">GOOGLE ACCOUNTS</span><h2>Gmail, Calendar and Drive</h2><p>Connect both Google accounts directly in this browser. Imported updates always enter the review queue.</p></div><span class="sync-state ${clientReady ? connected ? 'connected' : 'pending' : 'required'}">${clientReady ? connected ? `${connected} active this session` : 'Ready to authorize' : 'OAuth client ID required'}</span></div>
